...
1. `./scripts/agent_gate.sh` — canonical combined gate (stdout JSON, `schema_version: pixal3d-agent-gate/1`; parse with `2>/dev/null | jq -e .overall_ok`). Browser subprocess exit **1** with explicit ZeroGPU quota copy is a verified pass when `overall_ok` is true.
