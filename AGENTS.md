...
- **Post-recovery agent gate:** `./scripts/agent_gate.sh` (parity + health/HTML + browser E2E + JSON summary; browser exit **1** with explicit ZeroGPU quota copy is a verified pass). Optional `--url URL`. For health/HTML only, use `./scripts/verify_hosted_space.sh`. Do not combine browser gate with `--generate` in the same session. For optional full generate smoke, create a venv and install `scripts/smoke-requirements.txt` first (`gradio_client` is not in the Space runtime requirements). Use `--generate` to call `/generate_3d` with ZeroGPU-safe defaults (512 / 5 steps); on hosted ZeroGPU the smoke skips `/warmup_runtime` (same as the browser UI). Anonymous cold runs usually take ~2–3 minutes and return a geometry-only `glb_path`; sign in on the Space for preview frames and textured extract.
