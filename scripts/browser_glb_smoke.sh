...
for ((i = 0; i < TOTAL_WAIT; i += 3)); do
  ab eval "typeof window.__pixal3dSmokeAdvance === 'function' && window.__pixal3dSmokeAdvance()" >/dev/null 2>&1 || true
  tick="$(ab_text "document.body?.dataset?.smokeTick || ''")"
  if [[ -z "$tick" ]]; then
    tick="$(ab_text "typeof window.__pixal3dSmokeAdvance === 'function' ? window.__pixal3dSmokeAdvance() : ''")"
  fi
  [[ -z "$tick" ]] && tick="no-hook"
