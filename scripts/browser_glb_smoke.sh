...
for ((i = 0; i < TOTAL_WAIT; i += 3)); do
  tick="$(ab_text "typeof window.__pixal3dSmokeAdvance === 'function' ? (window.__pixal3dSmokeAdvance(), document.body.dataset.smokeTick || '') : (document.body.dataset.smokeTick || '')")"
  if [[ -z "$tick" && -n "$last" ]]; then
    tick="$last"
  fi
  [[ -z "$tick" ]] && tick="no-hook"
