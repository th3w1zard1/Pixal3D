...
class Pixal3DImageTo3DPipeline(Pipeline):
    ...
    def run(
        ...
        if pipeline_type == "1024":
            ...
            raise ValueError(
                f"Invalid pipeline type for Pixal3D proj mode: {pipeline_type}. "
        ...
        assert self.image_cond_model_shape_512 is not None, (
            "image_cond_model_shape_512 not set."
        )
        assert self.image_cond_model_shape_1024 is not None, (
            "image_cond_model_shape_1024 not set."
        )
        assert self.image_cond_model_tex_1024 is not None, (
            "image_cond_model_tex_1024 not set."
        )
        ...
        if self.low_vram:
            self.models["shape_slat_decoder"].to(self.device)
            self.models["shape_slat_decoder"].low_vram = True
        hr_coords = self.models["shape_slat_decoder"].upsample(
        ...
        while True:
            ...
            quant_coords = torch.cat(
                [
                    hr_coords[:, :1],
                    ((hr_coords[:, 1:] + 0.5) / lr_resolution * (grid_res - 1))
                    .round()
                    .int(),
                ],
                dim=1,
        ...
        noise_hr = SparseTensor(
            feats=torch.randn(
                ...
                self.models["shape_slat_flow_model_1024"].in_channels,
            ).to(self.device),
